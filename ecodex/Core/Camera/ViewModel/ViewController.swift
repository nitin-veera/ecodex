...
class ResultViewController: UIViewController {
    ...
    private var entryService = EntryService()
...
//    }
    ...
    @objc private func savePhoto() {
        ...
        if let image = capturedImage, let plantName = plantName, let location = currentLocation {
            // image -> url
            ImageUploader.uploadImage(image: image) { profileImageUrl in
                self.imageURL = profileImageUrl
            }
            // plantID
            plantService.fetchPlantIDByName(withScientificName: plantName) { ID in
                self.plantID = ID
            }
            if (self.plantID == nil) {
                print("plant is not in database")
                return
            }
            ...
            entryService.uploadEntry(plantID: self.plantID!, location: self.coordinate!, entryImage: self.imageURL!) { pass in
                if pass {
                    print("cool")
                } else {
                    return
